    ...
    <div v-show="currentValue">
        <div class="gesturePwd">
            <div class="gesture-box">
                ...
                <!-- <a ref="updatePwd" style="position: absolute;right: 5px;top: 5px;color:#fff;font-size: 13px;display:block;" @click="closePwd()">关闭</a> -->
                <canvas ref="canvas" style="background-color:#34353A; display: inline-block; margin-top: 5vh; width: 90vw; height: 90vw;"></canvas>
                <a ref="updatePwd" class="update-pwd-msg" @click="updatePassword()">忘记手势密码</a>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    created () {
        ...
        if (!this.accountOpenFlag) {
            this.accountOpenFlag = JSON.parse(sessionStorage.getItem('isAccountOpen'))
        }
        if (this.isUpdatePwd) {
            this.updatePwdFlag = this.isUpdatePwd
        }
    },
    mounted () {
    ...
    methods: {
        ...
        storePass (psw) {
            if (this.pswObj.step == 1) {
                if (this.openAccountMode === 'BIND_AND_OPEN_MODE') {
                    // eslint-disable-next-line
                    MtaH5.clickStat('open_open_szssmmy_qrssmm_click')
                } else if (this.openAccountMode === 'BIND_ONLY_MODE') {
                    // eslint-disable-next-line
                    MtaH5.clickStat('open_zhmsktlc_qrssmmy')
                }
                if (psw.length < 4) {
                    ...
                    if (this.checkPass(this.toPasswordString(this.pswObj.fPassword), this.toPasswordString(psw))) {
                        ...
                        this.$refs.gestureTitle.style.color = "#FFFFFF";
                        this.drawStatusPoint('#2CFF26');
                        this.drawPoint('#2CFF26');
                        window.localStorage.setItem('chooseType', this.chooseType);
                        // 两次密码一致后保存密码跳转至下一页面
                        let sha256 = require("js-sha256").sha256
                        let shaPassword = sha256(this.pwdSlat + this.toPasswordString(psw))
                        this.$emit('postSignPassword', shaPassword, this.updatePwdFlag);
                    } else {
                ...
                vm.$http.post('/salary/h5/check_gesture_password', { gesture_password: shaPassword }).then(
                    function (result) {
                        ...
                        if (vm.errorTimes >= 5) {
                            // console.log('密码输入错误已达次数：', vm.errorTimes)
                        }
                    }
                )
            } else {
                if (this.openAccountMode === 'BIND_AND_OPEN_MODE') {
                    // eslint-disable-next-line
                    MtaH5.clickStat('open_open_szssmmy_szssmm_click')
                } else if (this.openAccountMode === 'BIND_ONLY_MODE') {
                    // eslint-disable-next-line
                    MtaH5.clickStat('open_zhmsktlc_szssmmy')
                }
                if (psw.length < 4) {
                    this.drawStatusPoint('red');
                    this.drawPoint('red');
        ...
        makeState () {
            if (this.$refs.updatePwd && this.$refs.gestureTitle) {
                if (this.pswObj.step == 2) {
                    this.$refs.updatePwd.style.display = 'block';
                    var gestureTitle = this.$refs.gestureTitle;
                    gestureTitle.style.color = "#FFFFFF";
                    gestureTitle.innerHTML = '请输入手势密码';
                } else if (this.pswObj.step == 1) {
                    this.$refs.updatePwd.style.display = 'none';
                } else {
                    this.$refs.updatePwd.style.display = 'none';
                    var gestureTitle = this.$refs.gestureTitle;
                    gestureTitle.style.color = "#FFFFFF";
                    gestureTitle.innerHTML = '请绘制您的图形密码';
                }
            }
        ...
        updatePassword () {
            ...
            if (this.accountOpenFlag) {
                this.$router.push({
                    path: '/liveDetection',
                    name: 'LiveDetection',
                    params: { updatePwdFlag: this.updatePwdFlag }
                })
            }
        },
        // 组件重置
